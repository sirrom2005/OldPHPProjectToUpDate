...
$siteObj = new site();
$comObj = new commonDB();	
$credit = $comObj->getDataById("odb_credit_cost", $_POST['credit_id'] );

$_POST['credit_amount'] = $credit['credit'];
$_POST['card_amount'] 	= $credit['cost'];
/******************************************************************************
...
*******************************************************************************/
    ...
    */
    ...
    if ($pnp_post_values == "")
	{
		...
		$pnp_post_values .= "card-amount=" . $_POST['card_amount'] . "&";
		...
		$pnp_post_values .= "card-type=" . $_POST['card_type'] . "&";
        ...
        // MORE
		$pnp_post_values .= "currency-symbol=" . $_POST['currency_symbol'] . "&";
    ...
    */
	$_POST['MErrMsg']	= $pnp_transaction_array['MErrMsg'];
	$_POST['orderID']	= $pnp_transaction_array['orderID'];
	unset($_POST['publisher_name']);
    unset($_POST['publisher_email']);
		
	if ($pnp_handle_post_process != "no") 
	{
		if ($pnp_transaction_array['FinalStatus'] == "success")
		{
			$_POST['trans_states'] = "Successful payment";
			$siteObj->addTransaction($_POST);
			$_SESSION['TRANSACTION'] = $_POST;
			header("location: ../buy_credit_success.html");
		}
		elseif ($pnp_transaction_array['FinalStatus'] == "badcard")
		{
			$_POST['trans_states'] = "Badcard";
			$siteObj->addTransaction($_POST);
			header("location: ../index.php?action=buy_credit_error&id=".base64_encode(1));
		}
		elseif ($pnp_transaction_array['FinalStatus'] == "fraud")
		{
			$_POST['trans_states'] = "Fraud";
			$siteObj->addTransaction($_POST);
			header("location: ../index.php?action=buy_credit_error&id=".base64_encode(2));
		}
		elseif ($pnp_transaction_array['FinalStatus'] == "problem") 
		{
			$_POST['trans_states'] = "problem";
			$siteObj->addTransaction($_POST);
			header("location: ../index.php?action=buy_credit_error&id=".base64_encode(3));
		}
		else 
		{
			//this should not happen
			$_POST['trans_states'] = "Unkonw";
			$siteObj->addTransaction($_POST);
			header("location: ../index.php?action=buy_credit_error&id=".base64_encode(4));
		}
	}
?>
