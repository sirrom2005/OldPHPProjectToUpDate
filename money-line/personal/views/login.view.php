...
$('#changepinbtn').bind('togglecontent', function(event,mode)
{
    ...
    if (mode=='on') {
        ...
        if ($.trim(oldpin.val())=="")	{
            ...
            if ( isPINStartZero($.trim(pin.val())))	{
                msg = Raxan.getVar('pin-start-zero');
                pin.addClass('highlight');

                pin.focus();//
            }else
            if ( isSpecialSequence($.trim(pin.val())))	{
                msg = Raxan.getVar('pin-in-special-sequence');
                pin.addClass('highlight');
...
function resetFields()
{
    ...
    function isSpecialSequence(sText)
    {
        try
        {
            var num1, num2, num3, num4;

            num1 = parseInt(sText.charAt(0));
            num2 = parseInt(sText.charAt(1));
            num3 = parseInt(sText.charAt(2));
            num4 = parseInt(sText.charAt(3));

            //check for number occuring more than once
            if(((num1 == num2 && num1 == num3 ) ||( num1 == num3 && num1 == num4 )) ||
                (num1 == num2 && num1 == num4) ||
                (num2 == num1 && num2 == num4) || (num2 == num1 && num2 == num3) ||
                (num2 == num3 && num2 == num4) ||
                (num3 == num1 && num3 == num2) ||  (num3 == num1 && num3 == num4) ||
                (num3 == num2 && num3 == num4) )
                return true;
            //check for number occuring as 1234 ascending order
            if( (num1 == (num2-1)) && (num2 == (num3 -1)) &&
                (num3 == (num4 - 1)))
                return true;
            //check for number occuring as 4321 descending order
            if( (num1 == (num2+1)) && (num2 == (num3 +1)) &&
                (num3 == (num4 + 1)))
                return true;
...
<div class="login container c37">
    ...
    <div id="tipcontainer" class="tip left">
        <a id="tipbtn" class="right dn1" href="#" xt-bind="#click,nextTip,,,#tipbtn img">
            <img src="views/images/loader.gif" alt="." class="hide" style="margin-right:5px;"/>
            <span langid="next.tip">Next Tip</span>
        </a>
        <h3 id="tipheader" langid="security.tips">Security Tips</h3>
        <h3 id="tiptitle">Protect any recorded passwords</h3>
        <div id="tipcontent">
            <p>The strength of your password will help keep it good for a longer time. A password that is shorter than 8 characters should be considered only good for a week or so, while a password that is 14 characters or longer can be good for several years.</p>
        </div>
    </div>
    <img src="views/images/tip_rt.png" alt="." class="tip-rt left"  />
