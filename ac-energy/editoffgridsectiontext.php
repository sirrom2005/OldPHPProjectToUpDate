...
class Index extends common{
    ...
    protected function _load(){
        ...
        $rs = $this->db->table("off_grid_quote_list filename,text{$this->section}","id=?",$this->id); 
    ...
    protected function post(){
        ...
        try{
            $rt = $this->db->tableUpdate("off_grid_quote_list", $data, "id=?", $this->id);
            if($rt){
                @unlink('tmp/'.$this->key.'/quote.pdf');
                $this->flashmsg($this->icon.'Template text changed','fade','rax-box success');
                $this->redirectTo("off-grid-preview.php?id={$this->id}");
            }
        }catch(Exception $ex)
        {
