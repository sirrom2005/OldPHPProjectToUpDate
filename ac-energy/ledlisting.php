...
class Listing extends common {
    ...
    protected function _config(){
        parent::_config();
        $this->autoAppendView = 'ledlisting.view.php';  // set page view file name
    }
    ...
    protected function loadList(){
        ...
        { 
            ...
            $rows       = $this->db->query("SELECT count(a.id) FROM leds_info a INNER JOIN clients c ON c.id = a.clientid $query");
            $rowCount   = $rows->fetchColumn(0);
            $lower      = (($this->pgNumber-1) * $this->pageSize);
            $offset     = $this->pageSize;

            $rs = $this->db->query("SELECT a.id, CONCAT(c.title,' ',c.firstname,' ',c.lastname) as fullname, c.email, a.colour_tem, a.a19_light_bulb_cnt,a.outdoor_floodlights_cnt,a.date_added FROM leds_info a
    ...
    {
        $id = $e->intVal();
        if($this->db->tableDelete('leds_info','id=?',$id)){    
            @unlink("tmp/LEDINFO/ledinfo{$id}.pdf");
            $this->pgNumber   = 1;
    ...
    protected function downloadPdf($e){
        ...
        $file = "tmp/LEDINFO/ledinfo{$id}.pdf";
