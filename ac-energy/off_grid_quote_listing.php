...
class Listing extends common {
    ...
    protected function _config(){
        parent::_config();
        $this->autoAppendView = 'offgridquotelisting.view.php';  // set page view file name
    }
    ...
    protected function loadQuoteList(){
        ...
        { 
            ...
            $rows       = $this->db->query("SELECT count(a.id) FROM off_grid_quote_list a INNER JOIN clients c on c.id = a.clientid $query");
    ...
    {
        $id = $e->intVal();
        $rs = $this->db->table('off_grid_quote_list filename','id=?',$id);
        
        $rt = $this->db->exec("DELETE q,r FROM off_grid_quote_list q
                               INNER JOIN off_grid_section_quote r ON r.quote_id = q.id
                               WHERE q.id = '$id'");
    ...
    protected function sendEmail($e)
    {
        ...
        $rs = $this->db->execQuery("SELECT q.id, CONCAT(c.title,' ',c.firstname,' ',c.lastname) AS fullname, c.email FROM off_grid_quote_list q
                                    INNER JOIN clients c on c.id = q.clientid
                                    WHERE q.filename = '{$id[1]}'");
        
        if(!file_exists($file)){
            $this->flashmsg($this->icon.'PDF file not generated<br>Click <a href="getoffgridpdf.php?id='.$rs[0]['id'].'">here</a> to create pdf file','fade','rax-box error');
            return false;
        }
    ...
    protected function downloadPdf($e){
        ...
        if(file_exists($file)){
            ...
            $this->redirectTo("getoffgridpdf.php?id=".$id[1]);
        }
    }
