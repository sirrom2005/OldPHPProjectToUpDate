...
class DBController{
	...
	*/
	public function dbregister($email, $pass, $title, $flname, $phone, $address, $airline, $city, $hash, $country_code){
		...
		$airline 		= mysqli_real_escape_string($this->link,$airline);
		...
		$q = "insert into users(pk_user,email,title,flname,phone,address,airline,city,password,usr_confirm_hash,country_code,usr_ip) values('NULL','$email','$title','$flname','$phone','$address','$airline','$city','$pass','$hash','$country_code','$ip')";
	...
	**/
	...
	public function getUsersPerCountry($userkey){
		/*temporarily disable requirement that user be confirmed
    $q = "SELECT COUNT(*) AS value,users.country_code,country_name FROM users INNER JOIN Country ON Country.country_code = users.country_code WHERE usr_is_confirmed=1 and pk_user <> '$userkey' GROUP BY users.country_code";		
		*/
    $q = "SELECT COUNT(*) AS value,users.country_code,country_name FROM users INNER JOIN Country ON Country.country_code = users.country_code WHERE pk_user <> '$userkey' GROUP BY users.country_code";		
    $results = mysqli_query($this->link,$q);
		/* Error occurred, return given name by default */
		if(!$results || (mysqli_num_rows($results) < 1)){
			mysqli_free_result($results);
	...
	public function getUsersData($userkey){
		mysqli_query($this->link,"SET NAMES 'utf8'");	
    /*temporarily disable requirement that user be confirmed
		$q = "SELECT pk_user,country_name,email,title,flname,address,airline,city,phone,usr_ip,usr_nmb_logins,usr_signup_date,usr_is_blocked,usr_is_admin FROM users INNER JOIN Country ON Country.country_code=users.country_code WHERE usr_is_confirmed=1 and pk_user <> '$userkey'";		
		*/
    $q = "SELECT pk_user,country_name,email,title,flname,address,airline,city,phone,usr_ip,usr_nmb_logins,usr_signup_date,usr_is_blocked,usr_is_admin,usr_is_confirmed FROM users INNER JOIN Country ON Country.country_code=users.country_code WHERE pk_user <> '$userkey'";		
		$results = mysqli_query($this->link,$q);
		/* Error occurred, return given name by default */
		if(!$results || (mysqli_num_rows($results) < 1)){
			mysqli_free_result($results);
		...
		$aResults = array();
		while ($row = $results->fetch_assoc()) {
			$aResults[] = array( "pk_user"=>$row['pk_user'] ,"country_name"=>$row['country_name'] ,"email"=>$row['email'],"flname"=>$row['flname'],"title"=>$row['title'],"address"=>$row['address'],"airline"=>$row['airline'],"city"=>$row['city'],"phone"=>$row['phone'],"usr_ip"=>$row['usr_ip'],"usr_nmb_logins"=>$row['usr_nmb_logins'],"usr_signup_date"=>$row['usr_signup_date'],"usr_is_blocked"=>$row['usr_is_blocked'],"usr_is_admin"=>$row['usr_is_admin'],"usr_is_confirmed"=>$row['usr_is_confirmed']);
		}
		mysqli_free_result($results);
		return $aResults;
	}
