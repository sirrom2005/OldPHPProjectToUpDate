	...
	* */
	...
	public function processLogin(){
		...
		if($retval){
		    ...
		    //Redirect admin login to admin page
		    if($this->isAdmin()) {
	...
	public function processForgotPassword(){
		...
		if(!$email || mb_strlen($email = trim($email)) == 0){
			...
			/* Check if email is already in use */
				...
				$this->formcontroller->setError($field, "Email does not exist in the system");
			}
		}
	...
	public function processResetPassword(){
		...
		elseif(mb_strlen(trim($pass1)) < 5){
			$this->formcontroller->setError($field, "5 characters necessary");
		}
...
(debian)|(ns)|(download))$", $pass1))
	...
	public function __login($email, $pass, $subremember){
		...
		if($result == -1){
			...
			if($result2 == -1){
				$field = "email";
				$this->formcontroller->setError($field, "Sorry, your account has not yet been activated. Please <a href='contact.php'>contact us</a> for more information.");
				return false;
			}
			$result3 = $this->dbcontroller->is_blocked($email);
			if($result3 == 1){
				$field = "email";
				$this->formcontroller->setError($field, "Sorry, your account is suspended. Please <a href='contact.php'>contact us</a> for more information.");
				return false;
			}
		}
		/* Return if form errors exist */
	...
	public function processRegisterx(){
		...
		$retuserkey = $this->__register( $_POST['email'],$_POST['pass'],$_POST['title'],$_POST['flname'],$_POST['phone'],$_POST['address'],$_POST['airline'],$_POST['city'],$_POST['country'],$_POST['country_code'],$_POST['confemail'],$_POST['confpass']);      
		...
		/* Error found with form */
			...
			$json = array(
				...
				"errors" => array(
								...
								array("name" => "phone","value" => $this->formcontroller->error_value("phone")),
								array("name" => "address","value" => $this->formcontroller->error_value("address")),
								array("name" => "airline","value" => $this->formcontroller->error_value("airline")),
								array("name" => "city","value" => $this->formcontroller->error_value("city")),
	...
	*/
	public function __register($email, $pass, $title, $flname, $phone, $address, $airline, $city, $country, $country_code, $confemail, $confpass){



        if(!$airline || mb_strlen($airline = trim($airline)) == 0){
			$this->formcontroller->setError("airline", "Airline or company not entered");
		}
        if(!$city || mb_strlen($city = trim($city)) == 0){
			$this->formcontroller->setError("city", "City not entered");
		}   
    /*
        if(!$phone || mb_strlen($phone = trim($phone)) == 0){
			$this->formcontroller->setError("phone", "Telephone not entered");
		}
    */
...
(debian)|(ns)|(download))$", $flname))
		...
		*/
		...
		if(!$country || mb_strlen($country = trim($country)) == 0){
			...
			if($country_code == -1){ 
				...
				if($db_country_code == null){
					$this->formcontroller->setError($field, "Please enter a valid country");
				}
		...
		/* No errors, add the new account to the db*/
	        ...
	        if(($retuserkey = $this->dbcontroller->dbregister($email, sha1($pass), $title, $flname, $phone, $address, $airline, $city, $hash, $country_code)) > 0){	
				//if user was successfuly registered, lets send an email where he can activate his account!
				$this->mailer->sendForReg($email,$hash,$title.' '.$flname);
...
(debian)|(ns)|(download))$", $flname))
	...
	public function processAdminOperation(){
		...
		$operation = $_POST['op'];
    /* disabling original block action
		if($operation=='block'){
			if($currentvalue == '0')
    ...
    */
    //new block action will toggle the is_confirmed status instead
    if($operation=='block'){
			if($currentvalue == '0')
				$retval = $this->dbcontroller->updateUserField($userkey,'usr_is_confirmed',1);
			else
				$retval = $this->dbcontroller->updateUserField($userkey,'usr_is_confirmed',0);	
		}
