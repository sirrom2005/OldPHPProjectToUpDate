...
{	
	...
	function getUserList($data=array(),$homepage=false)
	{
		$limit = ($homepage)? 'LIMIT 12': '';
		$query = $innerQuery = "";
		...
		$hasImage = (empty($_SESSION['BBPINWORLD']['hasImage']))? 'AND g.image_name is null' : '';
		...
		$sql = "SELECT p.id 
				...
				$innerQuery
				WHERE 0=0 $query $hasImage GROUP BY p.id ORDER BY g.date_added DESC $limit";
	...
	function getGroupList($data=array(),$limit=null,$id=null)
	{
		$query  = "";
		$query .= (!empty($data['country_id']))? 'AND p.country_id = ' . $data['country_id'] . ' ' : '';
		$query .= (!empty($data['interest']))? 'AND gi.interest_id = ' . $data['interest'] . ' ' : '';
		$query .= (!empty($id))? 'AND p.account_id = '.$id : '';
	...
	{				
		$sql = "SELECT p.fname,p.lname,p.email,if(p.gender='M','Male','Female') AS gender,p.gender_prefrence AS gender_prefrence
				...
				,if(z.name IS NULL,'',CONCAT(' - (<a href=\"index.php?action=search&cz=',p.country_zone_id,'\">',z.name,'</a>)')) AS country_zone,
	...
	function getProfilePhotos($id,$limit=null)
	{				
		$limit = ($limit)? "limit $limit" : '' ;
		$sql = "SELECT id,image_name,profile_img,description FROM account_gallery WHERE account_id = '$id' ORDER BY profile_img DESC,date_added DESC $limit";
		return parent::executeQuery($sql, true);
	}
	...
	{			
		$query = (isValidEmail($user))? $query = "p.email = '$user'" : "pin.bbmpin = '$user'";
		
		$sql = "SELECT p.id,p.fname,p.lname,p.email
				,if(pin.id IS null,0,1) as bbm
				,if(p.gender IS null,0,1) as gender
				,if(p.dob IS null,0,1) as dob
				,if(ISNULL(g.id),0,1) as hasImage
				,(SELECT if(is_read=1,'no','yes') FROM messages WHERE receiver_id = p.id AND is_read = 0 LIMIT 1) as newMessage
				FROM people p 
				LEFT JOIN bbm_pin pin ON pin.account_id = p.id
				LEFT JOIN account_gallery g ON g.account_id = p.id
		...
		$rt = parent::executeQuery($sql, false);		
		if($rt){
			$rt['complete'] = (empty($rt['bbm']) || empty($rt['gender']) || empty($rt['dob']))? 0 : 1;
			unset($rt['bbm']);
			unset($rt['gender']);
			unset($rt['dob']);
	...
	{				
		$sql = "SELECT c.id,p.id AS senderid,g.account_id AS group_owner,CONCAT(p.fname,' ',p.lname) AS sendername,c.comment,c.date_added FROM group_comment c
				...
				LEFT JOIN people p ON p.id = c.sender_id
				WHERE c.group_id = '$id' ORDER BY c.date_added DESC LIMIT 20";
		return parent::executeQuery($sql, true);
	}
	...
	{				
		$sql = "SELECT 
				'New member' AS title,
				CONCAT(p.fname,' ',p.lname,' reqistered on ', DATE_FORMAT(p.date_added,'%Y-%m-%d')) AS info,
				CONCAT('profile_',p.id,'.html') AS url 
				FROM `people` p
				INNER JOIN `bbm_pin` b ON b.account_id = p.id
				INNER JOIN `account_gallery` g ON g.account_id = p.id
				GROUP BY p.id
				ORDER BY p.date_added DESC LIMIT 10";
		return parent::executeQuery($sql, true);
	}
}
?>