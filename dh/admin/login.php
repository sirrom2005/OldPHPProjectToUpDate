<?php
	...
	if( $_POST )
	{
		...
		if( !empty($rs) )
		{			
			if( $rs['account_type'] == 4 || $rs['account_type'] == 3 )
			{
				#######-- Pubsliher/Developer and MEMBERS--######
				$_SESSION['account_user'] = $rs;
				echo "<script> location='../'; </script>";
				echo "<meta http-equiv='refresh' content='0;../' />";
			}
			
			if( $rs['account_type'] == 2 ||  $rs['account_type'] == 1 )
			{
				#######-- ADMIN USER/STAFF ACCOUNT --######
				##~~~~~~~~~~~Check date time period~~~~~~~~~~~~~~##
				if(empty($rs['no_time_limit']))
				...
				{
					$_SESSION['admin'] = $rs;
					$comObj->logAdminActions("Login successful");
					echo "<script>location='".DOMAIN."admin/index.php?action=home'</script>";
				}
			}
		...
		{ 
			...
			$comObj->logAdminActions("LOGIN ATTEMP FAILED FOR USERNAME: $username AND PASSWORD: $password");
		}
	}
?>
