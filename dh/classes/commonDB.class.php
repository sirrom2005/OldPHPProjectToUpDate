...
class commonDB extends mySqlDB
{
	...
	function logAdminActions($action)
	{
		$time  = date("h:i:s a");
		$str = $_SESSION['admin']['id'].",".$_SESSION['admin']['user_name'].",$time,-$action-\n";
		$str2 = $_SESSION['admin']['id'].",".$_SESSION['admin']['user_name'].",$time,-$action-\n";
		$filename = date("Ymd").".csv";
		
		if(!file_exists(URL_PATH."admin/logs/$filename"))
		{
			$hder = "ADMIN ID, ADMIN USERNAME, TIME, ACTION\n";
			$str  = $hder.$str;
		}
		...
		/*FULL LOG*/	
		$ff = fopen(URL_PATH."admin/logs/adminlogs.csv", "a");
