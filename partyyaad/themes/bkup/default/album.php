...
<div id="main">
    ...
    <div id="padbox">
        ...
        <div id="images">
			...
			<?php
				//$ext = explode('.',$_zp_current_image->filename);
				//$ext = strtolower($ext[count($ext)-1]);
			?>
                <div class="image">
                	<div class="imagethumb">
                    	<a href="<?php echo 'cache/'.$_zp_current_image->album->name.'/'.substr($_zp_current_image->filename, 0,-4).'_595.jpg'; ?>" title="<?php echo getBareImageTitle(); ?>" class="lightview" data-lightview-group="album" ><?php printImageThumb(getAnnotatedImageTitle()); ?></a>
                        <span><a href="<?php echo html_encode(getImageLinkURL()); ?>#comment" class="addcomment" title="add comment">Comment</a> | 
                        <a href="<?php echo 'albums/'.$_zp_current_image->album->name.'/'.$_zp_current_image->filename; ?>" target="_blank" class="download" title="download full image">Download</a></span>
                    </div>                
                    <!--div class="imagethumb"><a href="index.php?p=image_window&a=<?php //echo urlencode($_zp_current_image->album->data['folder']);?>&i=<?php //echo substr($_zp_current_image->filename, 0,-4).'_595.'.$ext;?>" title="<?php //echo getBareImageTitle(); ?>" class="lightview" data-lightview-group="album" data-lightview-type="iframe" data-lightview-options="width:500, height:595, viewport:'scale'" ><?php //printImageThumb(getAnnotatedImageTitle()); ?></a></div-->
                </div>
            <?php endwhile; ?>
        </div>
