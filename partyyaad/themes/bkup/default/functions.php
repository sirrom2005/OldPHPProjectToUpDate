...
function getPhotoSlideShowLeft()
{
    $rs = query_full_array("SELECT i.filename,i.title,i.desc,a.folder FROM partyyaad_images i
							INNER JOIN partyyaad_albums a ON i.albumid = a.id
							WHERE 0=0 AND i.custom_data LIKE '%imageslideshow-left%' ORDER BY RAND() LIMIT 12");
    return $rs;
}

function getPhotoSlideShowRight()
{
    $rs = query_full_array("SELECT i.filename,i.title,i.desc,a.folder FROM partyyaad_images i
							INNER JOIN partyyaad_albums a ON i.albumid = a.id
							WHERE 0=0 AND i.custom_data LIKE '%imageslideshow-right%' ORDER BY RAND() LIMIT 12");
    return $rs;
}
...
{
    $rs = query_full_array("SELECT i.filename, i.title, i.desc,i.location, a.folder, a.title AS albumTitle FROM partyyaad_images i
                            INNER JOIN partyyaad_albums a ON i.albumid = a.id
...
function disPlayLeftBanner()
{
    $rs = query_full_array("SELECT i.filename, i.title, i.desc,i.location, a.folder, a.title AS albumTitle FROM partyyaad_images i
                            INNER JOIN partyyaad_albums a ON i.albumid = a.id
                            WHERE a.folder = 'banner_ads' AND i.custom_data LIKE '%L1%' ORDER BY RAND() LIMIT 1");
    return $rs;
}
function disPlayLeftBanner2()
{
    $rs = query_full_array("SELECT i.filename, i.title, i.desc,i.location, a.folder, a.title AS albumTitle FROM partyyaad_images i
                            INNER JOIN partyyaad_albums a ON i.albumid = a.id
                            WHERE a.folder = 'banner_ads' AND i.custom_data LIKE '%L2%' ORDER BY RAND() LIMIT 1");
    return $rs;
}
function disPlayLeftBanner3()
{
    $rs = query_full_array("SELECT i.filename, i.title, i.desc,i.location, a.folder, a.title AS albumTitle FROM partyyaad_images i
                            INNER JOIN partyyaad_albums a ON i.albumid = a.id
                            WHERE a.folder = 'banner_ads' AND i.custom_data LIKE '%L3%' ORDER BY RAND() LIMIT 1");
    return $rs;
}
function disPlayRightBanner()
{
    $rs = query_full_array("SELECT i.filename, i.title, i.desc,i.location, a.folder, a.title AS albumTitle FROM partyyaad_images i
                            INNER JOIN partyyaad_albums a ON i.albumid = a.id
                            WHERE a.folder = 'banner_ads' AND i.custom_data LIKE '%R1%' ORDER BY RAND() LIMIT 1");
    return $rs;
}
function disPlayRightBanner2()
{
    $rs = query_full_array("SELECT i.filename, i.title, i.desc,i.location, a.folder, a.title AS albumTitle FROM partyyaad_images i
                            INNER JOIN partyyaad_albums a ON i.albumid = a.id
                            WHERE a.folder = 'banner_ads' AND i.custom_data LIKE '%R2%' ORDER BY RAND() LIMIT 1");
    return $rs;
}
function disPlayRightBanner3()
{
    $rs = query_full_array("SELECT i.filename, i.title, i.desc,i.location, a.folder, a.title AS albumTitle FROM partyyaad_images i
                            INNER JOIN partyyaad_albums a ON i.albumid = a.id
                            WHERE a.folder = 'banner_ads' AND i.custom_data LIKE '%R3%' ORDER BY RAND() LIMIT 1");
    return $rs;
}
